...
AUI().add('speechrecognition', function (A) {
	var two_line = /\n\n/g;
	var one_line = /\n/g;
	var MIC = '/o/voice.command.portlet/image/mic.gif';
	var MIC_ANIMATE = '/o/voice.command.portlet/image/mic-animate.gif';
	var MIC_SLASH = '/o/voice.command.portlet/image/mic-slash.gif';
	...
	A.speechrecognition.SpeechRecognition = A.Base.create('speech-recognition', A.Base, [], {
		initializer: function () {
			...
			keyWord = this.get('keyWord').toLowerCase();
	...
	A.speechrecognition.start = function () {
		if(!recognizing){
			final_transcript = '';
			recognition.lang = themeDisplay.getLanguageId(); //'en-US';
			recognition.start();
			ignore_onend = false;
			final_span.innerHTML = '';
	...
	A.speechrecognition.readCustomCommands = function (speechCommand) {
		speechCommand = speechCommand.toLowerCase();
